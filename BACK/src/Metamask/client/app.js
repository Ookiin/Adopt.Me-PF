App = {
  ...
  loadEthereum: async () => {
    if (window.ethereum) {
      ...
      console.log(
        "No ethereum browser in installed. Try it installing Metamask"
      );
    }
  },
    ...
    for (let i = 1; i <= taskCounterNumber; i++) {
      ...
      let taskElement = `<div class="card bg-dark rounded-0 mb-2">
        ...
        taskDone && "checked"
          ...
          <p class="text-muted">Task was created ${
            (new Date(taskCreated * 1000), toLocaleString())
          }</p>
          </div>
      </div>`;
      html += taskElement;
